...
export class SceneRenderer {
  ...
  private cacheKey=''; private time=0; private px=0;private py=0;private lastScene='';private walk=0;
  private particles:Particle[]=[];
  actionAge=0;
  constructor(public canvas:HTMLCanvasElement){canvas.width=COLS*TILE;canvas.height=ROWS*TILE;this.c=canvas.getContext('2d')!;this.terrain.width=canvas.width;this.terrain.height=canvas.height;this.c.imageSmoothingEnabled=false;}
  burst(x:number,y:number,effect:string){this.actionAge=.35;for(let i=0;i<12;i++)this.particles.push({x:(x+.5)*TILE,y:(y+.6)*TILE,vx:Math.cos(i*2.4)*25,vy:-25-i*2,life:.5+i*.018,color:effect==='water'?'#b6e4e2':effect==='soil'?'#c89a69':effect==='food'?'#a4d5a3':'#f4d38b'});}
  ...
  private background(state:GameStateV2){
    ...
    if(scene==='home'){
      ...
      if(scene==='forest'){path(3,1,3,17);path(5,8,22,2);path(13,9,3,9)}
  ...
  draw(state:GameStateV2,dt:number,fishing:FishingSession|null,preview?:Appearance){
    ...
    for(const exit of scene.exits){const x=exit.x*TILE,y=exit.y*TILE;this.rect(x+2,y+5,20,14,'#5a6650');this.rect(x+4,y+6,16,10,'#d4c18d');this.label(exit.label,x+12,y-4,'#f4e8c6',false);}
    ...
    objects.push({y:this.py+1,draw:()=>drawPerson(c,this.px*TILE+12,this.py*TILE+21,preview??state.player.appearance,state.player.direction,this.walk,1,Math.sin(this.actionAge*35)*this.actionAge*8)});
    objects.sort((a,b)=>a.y-b.y).forEach(o=>o.draw());
    const target=targetTile(state);if(target.x>=0&&target.y>=0&&target.x<32&&target.y<20){c.strokeStyle='#f7e5ad';c.lineWidth=1;const tx=target.x*TILE,ty=target.y*TILE;for(const [x,y,dx,dy] of [[tx+2,ty+7,0,-5],[tx+2,ty+2,5,0],[tx+22,ty+17,0,5],[tx+22,ty+22,-5,0]]){c.beginPath();c.moveTo(x,y);c.lineTo(x+dx,y+dy);c.stroke();}}
  ...
  private label(text:string,x:number,y:number,color='#ede1be',background=true){const c=this.c;c.font='10px "Microsoft YaHei", sans-serif';c.textAlign='center';if(background){const w=c.measureText(text).width;this.rect(x-w/2-5,y-11,w+10,16,'#2b423adc')};c.fillStyle=color;c.fillText(text,Math.round(x),Math.round(y));}
  private water(o:WorldObject){const x=o.x*TILE,y=o.y*TILE,w=o.w*TILE,h=o.h*TILE;this.rect(x-3,y-3,w+6,h+6,'#7c9272');this.rect(x,y,w,h,'#497f86');this.rect(x+3,y+3,w-6,h-6,'#619c9e');this.rect(x+8,y+8,w-16,h-16,'#73aeb0');for(let i=0;i<o.w*o.h/2;i++){const px=x+10+(i*57+Math.floor(this.time*4))%(w-30),py=y+11+(i*43)%(h-20);this.rect(px,py,9+i%9,1,'#b6d1bd88')}
  ...
  private object(o:WorldObject,state:GameStateV2){const x=o.x*TILE,y=o.y*TILE,w=o.w*TILE,h=o.h*TILE,r=(a:number,b:number,d:number,e:number,f:string)=>this.rect(a,b,d,e,f);
    ...
    if(o.kind==='house'||o.kind==='shop'){
      ...
      if(o.kind==='house'){
        ...
        for(let i=0;i<3;i++){const sy=y-27-((this.time*7+i*13)%36);r(x+w-24+i*2,sy,7+i*2,5,'#e4dfc440')}
      }else{
  ...
  private plots(state:GameStateV2){const [w,h]=farmDimensions(state.upgrades.farm);const r=(a:number,b:number,d:number,e:number,f:string)=>this.rect(a,b,d,e,f);
    ...
    for(let y=0;y<10;y++)for(let x=0;x<12;x++){
      ...
      if(plot.crop){const def=CROPS.find(c=>c.id===plot.crop!.id)!,g=plot.crop.growth,ready=g>=def.days,dormant=!def.seasons.includes(seasonOf(state.calendar.day));
        ...
        if(g>0)r(px+7,py+14,10,5,ready?def.color:'#83a267');if(ready){r(px+8,py+16,9,5,def.color);r(px+10,py+12,4,3,'#ffffff55')}
  ...
  private weather(weather:'rain'|'snow'){for(let i=0;i<75;i++){const x=(i*131+this.time*(weather==='rain'?-25:5)+10000)%768,y=(i*67+this.time*(weather==='rain'?210:23))%480;this.rect(x,y,weather==='rain'?1:2,weather==='rain'?7:2,weather==='rain'?'#b9d4ca65':'#eef1e9bb')}}
}
