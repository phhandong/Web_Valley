...
export function validSave(raw:unknown):raw is GameStateV2{
  ...
  if(!obj(s.player)||!Object.hasOwn(SCENES,s.player.scene)||!int(s.player.x,1,30)||!int(s.player.y,1,18)||!['up','down','left','right'].includes(s.player.direction)||!passable(s.player.scene,s.player.x,s.player.y))return false;
...
export function migrateV1(raw:unknown):GameStateV2|null{
  ...
  state.inventory.slots=[{id:'ration',count:3}];
...
export function loadGame(storage:StorageLike):{state:GameStateV2;message:string;blocked:boolean}{
  try{
    ...
    return{state:initialState(),message:'欢迎来到溪谷。慢慢来，一颗种子就是开始。',blocked:false};
