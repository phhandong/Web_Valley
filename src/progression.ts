...
export const LEVEL_XP=[0,40,100,180,280,420,600,820,1080,1400];
export const AREAS=[
  {id:'grove',name:'萤火秘林',level:3,price:180,description:'成片野果、暮色萤火与每日林间补给。'},
  {id:'quarry',name:'回声石谷',level:5,price:400,description:'密集矿石、古老石碑与每日矿藏。'}
] as const;
...
export function purchaseArea(s:GameStateV2,id:string):Result{
  ...
  if(!a||nearby(s)?.id!==id||s.player.scene!=='forest')return{ok:false,message:'请走到森林的区域路牌旁办理通行。'};
