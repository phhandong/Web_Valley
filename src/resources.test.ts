...
describe('tree cutting and rock mining',()=>{
  it('cuts a whole tree and releases every occupied tile',()=>{
    ...
    expect(harvestResource(s,10,3).ok).toBe(true);expect(quantity(s.inventory,'wood')).toBe(6);expect(s.player.vitals.stamina).toBe(97);
  ...
  it('mines rocks, applies upgrades, and never gives the same reward twice',()=>{
    ...
    expect(harvestResource(s,8,8).ok).toBe(true);expect(quantity(s.inventory,'stone')).toBe(8);expect(s.player.vitals.stamina).toBe(99);
  ...
  it('does not spend stamina or clear scenery if the backpack cannot hold all materials',()=>{
    const s=tree();s.inventory.slots=Array.from({length:24},()=>({id:'wood',count:99}));s.inventory.slots[0].count=95;
  ...
  it('does not remove resources or give rewards if stamina is insufficient',()=>{
    const s=tree();s.player.vitals.stamina=2;const before=JSON.stringify(s);expect(harvestResource(s,10,3).ok).toBe(false);expect(JSON.stringify(s)).toBe(before);
  });
  it('restores a player inside a cleared tree and preserves clearing overnight',()=>{
    const s=tree();harvestResource(s,10,3);step(s,'up');expect(validSave(s)).toBe(true);
