...
export function gather(state:GameStateV2,x:number,y:number):Result{
  ...
  if(node.kind==='wood'&&state.selectedTool!=='axe')return result(false,'切换到斧头（6）收集木材');
  if(node.kind==='stone'&&state.selectedTool!=='pick')return result(false,'切换到石镐（7）采集石料');
...
export function cook(state:GameStateV2,id:string):Result{
  ...
  state.stats.cooked++;discover(state,id);return result(true,`${recipe.name}做好了`,'food');
}
...
export function canFish(state:GameStateV2){return SCENES[state.player.scene].fishing.some(([x,y])=>Math.abs(x-state.player.x)+Math.abs(y-state.player.y)<=1);}
export function acceptCatch(state:GameStateV2,replace=-1):Result{
  ...
  state.inventory=draft;state.pendingCatch=null;state.stats.caught++;discover(state,id);return result(true,`钓到了${ITEMS[id].name}！`,'harvest');
}
